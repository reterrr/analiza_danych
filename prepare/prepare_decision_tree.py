...
- Kodowanie kategorii: OrdinalEncoder (handle_unknown -> -1)
- Bez skalowania (drzewa są niewrażliwe na skalę)
- Podział strat. 70/15/15
- Zapis: X_*.npy (dense), y_*.npy; class_mapping.json; preprocessor.joblib
"""
...
def main():
    ...
    ap.add_argument("--outdir", default="prepared/decision_tree")
    ...
    X_enc = pre.fit_transform(X)  # gęste ndarray
    X_tr, X_va, X_te, y_tr, y_va, y_te = split_sets(X_enc, y)

    class_weights = compute_class_weight("balanced", classes=np.unique(y_tr), y=y_tr)
    id2class = {v: k for k, v in class2id.items()}

    np.save(outdir / "X_train.npy", X_tr)
    np.save(outdir / "X_val.npy", X_va)
    np.save(outdir / "X_test.npy", X_te)
    ...
    print("Kształty:", {"X_train": X_tr.shape, "X_val": X_va.shape, "X_test": X_te.shape})
